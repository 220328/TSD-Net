...
class CFU(nn.Module):
    ...
    def __init__(self, in_c, num_groups, hidden_dim=None):
        super().__init__()
...
class FPM(nn.Module):
    ...
    def forward(self, x2, pre, x1): # 参数分别表示较深层特征、当前预测图、较浅层特征；x2和pre同size，x1为两倍
        y_1 = x2 * pre
...
class DCTNet(BasicModelClass):
    def __init__(self, channel=16):
        ...
        # self.rfb4_1 = Octave(2048, 64)
        # self.rfb3_1 = Octave(1024, 64)
        # self.rfb2_1 = Octave(512, 64)
        ...
        # self.NCD2 = NeighborConnectionDecoder(channel)
        ...
        self.classifier0 = ConvBNReLU(channel, channel, 3, 1, 1)
    ...
    def body(self, o, a1, a2):
        ...
        # x4 = self.d1(x3_up + dct_feats_list[4])
        # x4_up = F.interpolate(x4, scale_factor=2, mode='bilinear', align_corners=False) # (batchsize, channel, 384, 384)
        # S_dct = self.classifier1(self.classifier0(x4)) 
        ...
        S_g = self.out_layer_01(self.out_layer_00(x3)) # 直接使用融合dct频域增强后的较浅层特征进行解码
        ...
        ra4_feat = self.RS5(x4_rfb + x0_fuse, guidance_g)
        ...
        ra3_feat = self.RS4(x3_rfb + x1_fuse, guidance_5)
        S_4 = ra3_feat
        ...
        ra2_feat = self.RS3(x2_rfb + x2_fuse, guidance_4)
        S_3 = ra2_feat
        ...
        # ra1_feat = self.RS2(x1_rfb + x3_fuse, guidance_3) # 融合dct频域增强后的较浅层特征
        # S_2 = ra1_feat + guidance_3
  
        # S_2_pred = F.interpolate(S_2, scale_factor=4, mode='bilinear')   # Sup-4 (bs, 1, 96, 96) -> (bs, 1, 352, 352)
